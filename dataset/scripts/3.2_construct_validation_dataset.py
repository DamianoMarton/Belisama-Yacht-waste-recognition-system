...
VALIDATION_IMAGES_FOLDER = os.path.join("dataset", "images", "val")
VALIDATION_LABELS_FOLDER = os.path.join("dataset", "labels", "val")

with open(MAIN_REFERENCE_FILE_PATH, "r", encoding="utf-8") as f:
    main_reference = json.load(f)

VALIDATION_DATASET_SIZE = 800  # total number of images in the validation dataset
...
DISTRIBUTIONS = {"number_of_elements": {"min": 0, "max": 3}, #uniform distribution
                 ...
                 "squeeze_factor": {"mean": 0.6, "std": 0.2}  # vertical squeeze factor for masses of algae
                 } 
...
for N in range(VALIDATION_DATASET_SIZE):
    print(f"Creating validation image {N+1}/{VALIDATION_DATASET_SIZE}")
    ...
    new_name = f"validation_image_{N:04d}"
    cv2.imwrite(os.path.join(VALIDATION_IMAGES_FOLDER, f"{new_name}.jpg"), final_bgr)

    with open(os.path.join(VALIDATION_LABELS_FOLDER, f"{new_name}.txt"), "w") as f:
        for label in labels:
    ...
    if N % ((VALIDATION_DATASET_SIZE - 1)//4) == 0:  # show 5 samples
        for label in labels:
            class_id, x_b, y_b, w_b, h_b = label
